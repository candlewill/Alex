...
def as_project_path(path):
    print("Run function as_project_path, whoes path is: %s" % path)
...
def _expand_file_var(text, path):
    ...
    # syntax.
    old, new = '__file__', "'{p}'".format(p=path)
    ...
    if not path.endswith('.py'):
        if force:
            ...
            with codecs.open(path, 'rb', encoding=encoding) as orig_file:
                text = orig_file.read()
            text = _expand_file_var(text, path)  # 替换变量
            print(text)
            # exit()
...
class Config(object):
    ...
    print("DEFAULT_CFG_PPATH: ", DEFAULT_CFG_PPATH)
    ...
    def getpath(self, path, default=None):
        ...
        print("path_componets: %s" % path_components)
