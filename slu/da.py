...
from slu.exceptions import SLUException, DialogueActException, DialogueActItemException
from ml.exceptions import NBListException
from ml.features import Abstracted
from ml.hypothesis import Hypothesis, NBList, ConfusionNetwork
from utils.text import split_by
from functools import reduce

'''
DialogueAct, DialogueActNBList类会被example.shub.py使用
'''
    ...
    def __str__(self):
        ...
        if str_self is None:
            try:
                ...
                print("self._value: %s" % self._value)
                print("*-" * 100)
        ...
        print("The type of self._str is %s, the content is: %s" % (type(self._str), self._str))
        return self._str
...
class DialogueAct(object):
    ...
    def __cmp(self, other):
        print("Start Comparing DialogueAct...")
...
class DialogueActNBList(SLUHypothesis, NBList):
    ...
    def merge(self):
        ...
        print("The type of n_best is: %s, length is: %s" % (type(new_n_best), len(new_n_best)))
        print("The type of element in n_best is: %s" % type(new_n_best[0]))
