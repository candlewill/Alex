...
from utils.config import load_as_module
...
class Ontology(object):
    ...
    # 判断name槽位，是否是binary取值的槽位
    # 判断依据是，依据ontology中的slot_attributes判断，如果"binary"存在与槽位中，则是
    ...
    # 看看用户最后一个DAI符合哪一种模式，模式定义在ontology中
    ...
    def last_talked_about(self, da_type, name, value):
        ...
        for target_slot, target_values in self.ontology['last_talked_about'].items():
            for target_value, source_patterns in target_values.items():
    ...
    # 获取某个槽位的兼容取值，如果不存在则，返回空集set()
    # 注意：所谓兼容取值，是指这个槽位可以取哪些值
    ...
    # 判断某个槽位的两个取值是否兼容
    # 兼容的条件是：val2为空，或者，val1存在于slot_pair的兼容取值中
