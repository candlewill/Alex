...
from dm.base import DiscreteValue, DialogueState
from dm.exceptions import DeterministicDiscriminativeDialogueStateException
from slu.da import DialogueAct, DialogueActItem, DialogueActConfusionNetwork
...
class D3DiscreteValue(DiscreteValue):
    ...
    def __init__(self, values=None, name="", desc=""):
        super(D3DiscreteValue, self).__init__(values, name, desc)
        self.name = name
        self.desc = desc
    ...
    # 返回values中value对应的概率，如果value不存在，则返回default_prob
    ...
    # 返回排序好的(value, prob)，按照prob从大到小排序
    def items(self):
    ...
    # 如果value是string，那么，将values中的某个value其对应的概率值设置为prob，set之后可能是的概率没有正规化
    # 如果value是dict，那么，将value赋值给values
    ...
    def normalise(self):
        ...
        s = sum([v for v in self.values.values()])  # 概率值的和
        if s < 1e-9:  # 接近零，均分
            # this is a backup solution with unknown consequences
            n = len(self.values)
            for value in self.values:
                self.values[value] = 1.0 / n if n != 0 else 0
        else:
            for value in self.values:
                self.values[value] /= s  # s!=0

    # 使用权值weight对概率值进行缩放，缩放方法是：weight*prob
    ...
    # 增加某个value的概率大小
    # 因为values的类型是defaultdict，默认值是float（即0.0），因此，如果不存在，则是在0的基础上增加，已存在，则是在已存在的概率值上增加
    ...
    def distribute(self, value, dist_prob):
        ...
        value_prob = self.values[value]  # value对应的概率值
        non_value_prob = sum([p for v, p in self.values.items() if v != value])  # 不是value的其他取值，对应的概率大小的总和

        # first deny the value proportionally to the denied probability
        self.set(value, (1.0 - dist_prob) * value_prob)  # 重新设置value对应的概率取值，(1-dist_prob)*原始概率
        ...
        for v in self.values:
            if v != value:  # 重新设置非value的其他取值
                if non_value_prob > 1e-9:  # 不接近0
...
class DeterministicDiscriminativeDialogueState(DialogueState):
    ...
    def __init__(self, cfg, ontology):
        ...
        self.turns = []
        self.turn_number = 0
        self.debug = cfg.getpath('DM/basic/debug', False)  # 路径DM/basic/debug存在于config文件中，则取对应的值，否则取False
        print("The value of self.debug is %s" % self.debug)
    ...
    def restart(self):
        ...
        if 'variables' in self.ontology:
            print("variables is in ontology")
    ...
    def _update_state(self, user_da, system_da):
        ...
        print("The type of user_da is: %s" % type(user_da))
        for prob, dai in sorted(user_da.items()):
            ...
            if dai.dat == "inform":
                ...
                if dai.name == "time":
                    ...
                    # print "#52", self.slots
        print("After DS update:")
        print(print(self.log_state()))
        print("-" * 50)
