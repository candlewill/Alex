...
class PTIENHDCPolicy(DialoguePolicy):
    ...
    def get_da(self, dialogue_state):
        ...
        slots_tobe_confirmed = dialogue_state.get_slots_tobe_confirmed(self.policy_cfg['confirm_prob'],
                                                                       self.accept_prob)
        ...
        # topic-dependent
            ...
            # talk about weather
            w_da = self.get_weather_res_da(dialogue_state, last_user_dai_type, slots_being_requested,
                                           slots_being_confirmed,
            ...
            t_da = self.get_connection_res_da(dialogue_state, last_user_dai_type, slots_being_requested,
                                              slots_being_confirmed,
                                              accepted_slots, changed_slots, has_state_changed)
            res_da.extend(t_da)
            res_da = self.filter_iconfirms(res_da)
    ...
    def get_requested_alternative(self, ds, slots_being_requested, accepted_slots):
        ...
        if 'route_alternative' not in ds:
            ...
            if slots_being_requested:
                res_da.append(
                    DialogueActItem('inform', 'alternative', word_for_number(ds['route_alternative'] + 1, True)))
    ...
    def gather_connection_info(self, ds, accepted_slots):
        ...
        has_from_place = from_stop_val != 'none' or from_street_val != 'none' or from_city_val not in ['none',
                                                                                                       'New York']
        ...
        default_city = self.ontology.get_default_value(
            'city')  # don't iconfirm borough if new york is the other city, because all boroughs are in new york
        ...
        from_streets = " and ".join(
            [street for street in [from_street_val, from_street2_val] if street not in ['none', None]])
        to_streets = " and ".join(
            [street for street in [to_street_val, to_street2_val] if street not in ['none', None]])
    ...
    def gather_weather_info(self, ds, accepted_slots):
        ...
        if in_city_val != 'none' and in_state_val == 'none':
            ...
            if not in_states or not len(in_states):
                print
                "WARNING: there is no state compatible with this city: " + in_city_val
    ...
    def req_distance(self, dialogue_state):
        ...
        steps = [(step.num_stops, step.departure_stop, step.vehicle, step.line_name) for step in leg.steps if
                 step.travel_mode == step.MODE_TRANSIT]
    ...
    def check_directions_conflict(self, wp):
        ...
        if wp.to_stop not in self.ontology.ontology[u'slots'][u'stop'] or wp.from_stop not in \
                self.ontology.ontology[u'slots'][u'stop']:
            return None
        # TODO: This is artificially added because boroughs now share city slot and we don't need to check borough compatibility
        if wp.to_city in self.ontology.ontology[u'slots'][u'borough'] or wp.from_city in \
                self.ontology.ontology[u'slots'][u'borough']:
            return None
    ...
    def fix_stop_street_slots(self, changed_slots):
        ...
        from_stop_value = ' and '.join(
            [changed_slots.pop(slot).mpv() for slot in from_list_stop if slot in changed_slots])
        to_stop_value = ' and '.join([changed_slots.pop(slot).mpv() for slot in to_list_stop if slot in changed_slots])
        from_city_value = ' and '.join(
            [changed_slots.pop(slot).mpv() for slot in from_list_city if slot in changed_slots])
