...
import codecs
import os
from utils.config import to_project_path

'''
本程序的目的是，构造Ontology

先手工构造大体结构，然后同程序的方式，自动填值

处理csv文件资源
'''

# tab-separated file containing street + city + lon|lat coordinates + slot_specification
STREETS_TYPES_FNAME = './data/streets.types.csv'
# tab-separated file containing stop + city + lon|lat coordinates
GENERAL_STOPS_LOCATIONS_FNAME = './data/stops.locations.csv'
BOROUGH_STOPS_LOCATIONS_FNAME = './data/stops.borough.locations.csv'
# tab-separated file containing city + state + lon|lat coordinates
CITIES_LOCATIONS_FNAME = './data/cities.locations.csv'
...
ontology = {
    ...
    'context_resolution': {
        ...
        'city': set(['in_state', 'from_city', 'to_city', 'via_city', 'in_city', 'from_stop', 'to_stop', 'from_street',
                     'from_street2', 'to_street', 'to_street2', ]),
    ...
    # 看看是否da_type, name, value是否和三元组内容，一一对应，如果对应，则取出slot、value
    'last_talked_about': {
        ...
        'lta_time': {
            ...
            # 三元组：source_dat, source_name, source_value，如果某个取值是""，则说明无条件满足
    ...
    # 限制了每个槽位可以取哪些值，虽然格式和slots相同，但是，槽位名称没有重复
    'compatible_values': {
        'stop_borough': {},
        'borough_stop': {},
        'street_borough': {},
...
'''
初始ontology构造完成，下面的代码使用程序的方式，从文件中读取数据，自动增加到初始Ontology中
'''


# 从database中增加槽位
# slot：需要增加的槽位，此槽位名称应该已存在
# category：存储了对应槽位需要增加的取值标签
# exceptions：定义了哪些取值不要新增进去
    ...
    with codecs.open(fname, 'r', 'UTF-8') as fh:
        for line in fh:
            ...
            if prev_value and prev_value != street_type and not surpress_warning:
                print(
                    'WARNING: slot ' + val_slot1 + " already contains " + prev_value + " (overwriting with " + type + ")!")
...
if __name__ == '__main__':
    ...
    def dict_2_str(dic):
        ...
        for key, value in dic.items():
            out[str(key)] = nondict_2_str(value) if type(value) != dict else dict_2_str(value)
        return out


    with open('ontology.json', 'w') as f:
        json.dump(dict_2_str(ontology), f, ensure_ascii=False, indent=
        4, sort_keys=True)
